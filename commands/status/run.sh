...
BANK="${HINDSIGHT_BANK:-}" API="" TASK=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --bank) BANK="$2"; shift 2 ;;
    --api) API="$2"; shift 2 ;;
    --task) TASK="$2"; shift 2 ;;
    -h|--help) echo 'status [--bank <id>] [--api <url>] [--task <root-id>]'; exit 0 ;;
    *) echo "unknown flag: $1" >&2; exit 2 ;;
  esac
done
[[ -n "$BANK" ]] || { echo 'set HINDSIGHT_BANK or pass --bank' >&2; exit 2; }
hs_connect
exec python3 "$PACK_DIR/assets/scripts/ship-report.py" status "$API" "$BANK" "$TASK"
